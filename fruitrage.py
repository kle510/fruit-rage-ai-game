...
class FruitRage:
    ...
    def possible_moves(self, state):
        ...
        potential_util = [self.outcome(state, move).utility for move in possible_moves]
        #print ("potential util: ", potential_util)

        if state.turn == 'MAX':
            sorted_moves = [x for _, x in sorted(zip(potential_util, possible_moves), key=lambda pair: pair[0], reverse=True)]

        else:
            sorted_moves = [x for _, x in sorted(zip(potential_util, possible_moves), key=lambda pair: pair[0])]
...
def alpha_beta(state, game):
    ...
    def max_value(state, alpha, beta, depth):
        if cutoff_test(state, depth):
            return eval(state)
        for move in game.possible_moves(state):
            alpha = max(alpha, min_value(game.outcome(state, move), alpha, beta, depth + 1))
            if alpha >= beta:
                return beta
        return alpha
    ...
    def min_value(state, alpha, beta, depth):
        if cutoff_test(state, depth):
            return eval(state)
        for move in game.possible_moves(state):
            beta = min(beta, max_value(game.outcome(state, move), alpha, beta, depth + 1))
            if beta <= alpha:
                return alpha
        return beta
...
def play_game():
    ...
    board_dimensions, fruit_types, p1_remaining_time, board = read_file(in_file)
    ...
    while True:
        ...
        p1_result = mini_max(curr_state(utility=0, board=p2_state.board, turn='MAX'), p1_game)
        ...
        write_play_file(board_dimensions, fruit_types, p2_remaining_time, p1_state.board)

        # player 2's turn

        board_dimensions, fruit_types, p2_remaining_time, board = read_file(in_file)
        ...
        p2_result = alpha_beta(curr_state(utility=0, board=p1_state.board, turn='MAX'), p2_game)
        ...
        write_play_file(board_dimensions, fruit_types, p1_remaining_time, p2_state.board)
...
def main():
    ...
    in_file = "input.txt"
    board_dimensions, fruit_types, remaining_time, board = read_file(in_file = "input.txt")
